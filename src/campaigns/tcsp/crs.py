...
def _ingestion(result, bucket, fdate, type):
    time, altitude, longitude, latitude, refdata = readCRS(result, bucket, type)
    data = {
        'time': time,
        'alt': altitude,
        'lon': longitude,
    ...
    #obtain heading, pitch, roll from ER2 data
    dfER2 = FlightTrackReader().process_tracks(fdate, False)

    merged_df = pd.merge(dfER2, df, left_on="timestamp",right_on="time", how="inner")
    # print("merged_df>>>>\n",merged_df)
    return merged_df

def _transformation(df: pd.DataFrame, rad_range):
...
def _integration(data: pd.DataFrame, fdate):
    ...
    # path creation
    zarr_path = create_zarr_dir(fdate)
...
def process_CRS():
    #rad_range values taken from olympex
    ...
    date = ""
    content_type = {"REF", "VEL", "IWC"}
    s3_file = s3fs.S3FileSystem()
    for type in content_type:
        keys = {}
        for obj in s3bucket.objects.filter(Prefix=f"tcsp/instrument-raw-data/CRS/CR"):
            if (type in obj.key):
                date = obj.key.split("/")[3].split("_")[0][2:]
                date = f"20{date}"
                if date not in keys:
                    keys[date] = []
                keys[date].append(obj.key)
        
        for fdate in keys:
            ingested_data = _ingestion(keys[fdate], bucket, fdate, type)
            transformed_data = _transformation(ingested_data, rad_range)
            integrated_data = _integration(transformed_data, fdate)
            point_clouds_folder = prep_visualization(integrated_data)      
            output_directory = f"{bucket}/tcsp/instrument-processed-data/CRS/{fdate}/{type}"
            s3_file.put(point_clouds_folder, output_directory, recursive= True)
