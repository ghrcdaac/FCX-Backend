def collectAvailabilityDateTimeRange(fileGroup):
    ...
    date = fileGroup[0].split("/")[-1].split("_")[2] + fileGroup[0].split("/")[-1].split("_")[3]
    formatted_date = '{}-{}-{}'.format(date[:4], date[4:6], date[6:])
    for index, filename in enumerate(fileGroup):
        starttime = filename.split("/")[-1].split("_")[4]
        if (index == end_index):
            endtime = str((int(starttime) + 200) if (int(starttime) + 200) < 235900  else 235900 )
            if (int(endtime) < 100000):
                endtime = f"0{int(endtime):05d}"
            print(endtime)
        else:
            endtime = fileGroup[index + 1].split("/")[-1].split("_")[4]
