...
def addDelta(base_time, seconds):
    # Convert base_time to datetime object
    base_datetime = datetime.fromisoformat(base_time)

    # Ensure seconds is of type int
    seconds = int(seconds)

    # Calculate timedelta using seconds
    delta = timedelta(seconds=seconds)

    # Add delta to base time
    combined_datetime = base_datetime + delta

    # Return combined datetime in ISO 8601 format
    return combined_datetime.isoformat(sep='T', timespec='auto')


def ingest(folder, df, base_time):
    """
